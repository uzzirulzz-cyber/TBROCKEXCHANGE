...
export function HomeView() {
  ...
  // Fetch wallet data for logged-in users
  useEffect(() => {
    if (!user) return;
    Promise.all([
      ...
      .then(([w, t]) => {
        ...
        setRecentTrades((t.trades || []).slice(0, 5));
      })
      .catch(() => {});
  }, [user]);
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              UID: <span className="font-mono text-[#0A84FF]">{user.uid}</span> · VIP: <span className="text-[#FF9F0A]">{
                user.vipLevel === 1 ? "🥉 VIP-1" : user.vipLevel === 2 ? "🥈 VIP-2" : user.vipLevel === 3 ? "🥇 superVIP" : "💎 Prime_Diamond"
              }</span>
            </p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
            ...
            <div className="text-white text-4xl font-bold mt-1 tabular-nums">
              {fmtMoney(ov?.totalAssetValue ?? 0)}
            </div>
            <div className="text-white/60 text-xs mt-1">
              Available: {fmtMoney(ov?.availableBalance ?? 0)} · Frozen: {fmtMoney(ov?.frozenFunds ?? 0)}
            </div>
            {/* Quick actions */}
            <div className="grid grid-cols-4 gap-3 mt-5">
              {[
                ...
                { icon: CandlestickChart, label: "Trade", onClick: () => navigate("trade") },
          ...
          <div className="grid grid-cols-3 gap-3">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              className="bx-glass rounded-2xl p-4 text-center">
              <div className="text-xs text-muted-foreground">Total Trades</div>
              <div className="text-xl font-bold text-white mt-1">{recentTrades.length}</div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
              className="bx-glass rounded-2xl p-4 text-center">
              <div className="text-xs text-muted-foreground">Profit</div>
              <div className="text-xl font-bold mt-1" style={{ color: (ov?.totalProfitGenerated ?? 0) >= 0 ? "#30D158" : "#FF453A" }}>
                {(ov?.totalProfitGenerated ?? 0) >= 0 ? "+" : ""}{fmtMoney(ov?.totalProfitGenerated ?? 0)}
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              className="bx-glass rounded-2xl p-4 text-center">
              <div className="text-xs text-muted-foreground">KYC</div>
              <div className="text-sm font-bold mt-1" style={{ color: user.kycStatus === "VERIFIED" ? "#30D158" : "#FF9F0A" }}>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate("markets")} className="text-xs text-[#0A84FF]">See All</button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {COINS.slice(0, 6).map((coin, i) => {
                ...
                const up = p.change >= 0;
                return (
                  <button key={coin.symbol} onClick={() => navigate("trade")}
                    className="bx-glass rounded-2xl p-4 flex items-center gap-3 hover:bx-glow transition-all text-left">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0" style={{ background: coin.color }}>
                      {coin.icon}
                    </div>
                    ...
                    <div className="text-right">
                      <div className="text-sm font-bold text-white tabular-nums">${formatPrice(p.price, coin.pair)}</div>
                      <div className={`text-xs font-medium ${up ? "text-[#30D158]" : "text-[#FF453A]"}`}>
          ...
          {recentTrades.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => navigate("history")} className="text-xs text-[#0A84FF]">View All</button>
              </div>
              <div className="bx-glass rounded-2xl overflow-hidden">
                {recentTrades.map((t, i, arr) => (
                  <div key={t.id} className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: i === arr.length - 1 ? "none" : "1px solid rgba(255,255,255,0.05)" }}>
                    <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: t.direction === "UP" ? "rgba(48,209,88,0.15)" : "rgba(255,69,58,0.15)" }}>
                      {t.direction === "UP" ? <ArrowRight className="w-4 h-4 rotate-[-45deg] text-[#30D158]" /> : <ArrowRight className="w-4 h-4 rotate-[45deg] text-[#FF453A]" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-white">{t.symbol}</div>
                      <div className="text-xs text-muted-foreground">{t.duration}s · {new Date(t.createdAt).toLocaleDateString()}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold" style={{ color: Number(t.profit) >= 0 ? "#30D158" : "#FF453A" }}>
          ...
          <div className="grid grid-cols-2 gap-3 pb-6">
            <button onClick={() => navigate("kyc")} className="bx-glass rounded-2xl p-4 flex items-center gap-3 hover:bx-glow transition-all">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: "rgba(48,209,88,0.15)" }}>
                <Shield className="w-5 h-5 text-[#30D158]" />
              </div>
              <div className="text-left">
                <div className="text-sm font-medium text-white">Verify Identity</div>
                <div className="text-xs text-muted-foreground">KYC verification</div>
              </div>
            </button>
            <button onClick={() => navigate("messages")} className="bx-glass rounded-2xl p-4 flex items-center gap-3 hover:bx-glow transition-all">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: "rgba(10,132,255,0.15)" }}>
                <Headset className="w-5 h-5 text-[#0A84FF]" />
              </div>
              <div className="text-left">
                <div className="text-sm font-medium text-white">Help Line</div>
  ...
  // ===== GUEST LANDING PAGE (marketing content) =====
  return (
